...
define(function(require, exports, module) {
	...
	utils.extend(exports, {
		...
		/**
		 * Find resource that matches given name either with
		 * strict or fuzzy match
		 *
		 * XXX too much find* methods, should rename them?
		 * 
		 * @param  {String} name    Resource name
		 * @param  {String} syntax  Section name (syntax)
		 * @param  {[type]} options 
		 * @return {Object}
		 */
		find: function(name, syntax, options) {
			var result = this.findSnippet(syntax, name);
			if (!result && options && options.fuzzySearch) {
				result = this.fuzzyFindSnippet(syntax, name, options.fuzzyScore);
			}
			return result;
		},
